...
def validate(state, allow_legacy=False):
    if isinstance(state, dict) and state.get('version') == 1:
        ...
        state = dict(state, version=2, shell={'windows': 'powershell', 'linux': 'bash'}.get(state.get('platform')),
                     flashbang=True, statusline=True)
...
def main():
    ...
    parser.add_argument('--statusline', choices=('true', 'false'))
    ...
    if args.action == 'read':
        ...
        if args.format == 'json':
            ...
            for key in ('flashbang', 'statusline', 'semantic_retrieval'):
    ...
    state = validate(dict(version=2, shell=args.shell, client=args.client,
                          flashbang=True if args.flashbang is None else args.flashbang == 'true',
                          statusline=True if args.statusline is None else args.statusline == 'true',
