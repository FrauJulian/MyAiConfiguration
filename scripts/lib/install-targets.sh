...
read_install_shell() {
  ...
  if [ -n "$shell" ]; then
    case "$shell" in
      powershell|PowerShell) printf 'powershell\n'; return 0 ;;
      bash|Bash) printf 'bash\n'; return 0 ;;
      *) printf 'Unknown --shell value: %s (expected powershell or bash)\n' "$shell" >&2; return 1 ;;
    esac
  fi
  read_single_selection 'Select shell' 'PowerShell' 'powershell' 'Bash' 'bash'
}
...
read_single_selection() {
  ...
  while :; do
    ...
    for ((index=0;index<${#labels[@]};index++)); do
      if [ "$index" -eq "$selected" ]; then printf '> %s\n' "${labels[$index]}" >&2; else printf '  %s\n' "${labels[$index]}" >&2; fi
    done
    printf 'Up/Down Navigate   Enter Confirm\n' >&2
    IFS= read -rsn1 key || return 1
    case "$key" in
      $'\x1b')
        read -rsn2 key || return 1
        case "$key" in
          '[A') selected=$(( (selected+${#labels[@]}-1)%${#labels[@]} ));;
          '[B') selected=$(( (selected+1)%${#labels[@]} ));;
        esac;;
      '') clear_interactive; printf '%s\n' "${values[$selected]}"; return;;
    esac
  done
}
...
# get_install_targets <generated> <home> <shell> <client> -- prints "source|dest" per line
get_install_targets() {
