        ...
        for shell, executable in SHELLS:
            ...
            with self.subTest(shell=shell), tempfile.TemporaryDirectory() as directory:
                ...
                if shell == 'powershell':
                    ...
                    commands = [
                        '& ./scripts/session-state.ps1 -Workspace $env:TASK_WORKSPACE -Goal $env:TASK_GOAL -Decisions @("keep", "line two") -Pending @("check")',
                        '& ./scripts/session-state.ps1 -Workspace $env:TASK_WORKSPACE -Verified @("passed")',
                        '& ./scripts/session-state.ps1 -Workspace $env:TASK_WORKSPACE -Pointer',
                    ]
                else:
                    prefix = [executable, '-c']
                    commands = [
                        'bash ./scripts/session-state.sh --workspace "$TASK_WORKSPACE" --goal "$TASK_GOAL" --decisions keep "line two" --open check',
                        'bash ./scripts/session-state.sh --workspace "$TASK_WORKSPACE" --verified passed',
                        'bash ./scripts/session-state.sh --workspace "$TASK_WORKSPACE" --pointer',
                    ]
